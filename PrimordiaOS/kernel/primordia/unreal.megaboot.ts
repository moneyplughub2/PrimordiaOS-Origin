...
// ---------- Core Primordia State ----------
interface PrimordiaState {
...
const initialState: PrimordiaState = {
    evolutionLevel: 1,
...
const MemoryEngine = {
    ...
    pushLore(state: PrimordiaState, item: any) {
        state.memory.lore.push({
            item,
            ts: Date.now()
        });
    },
    pushEvent(state: PrimordiaState, event: any) {
...
class PrimordiaKernel {
    state: PrimordiaState = structuredClone(initialState);
    ...
    private handleUnrealCommand(cmd: any) {
        ...
        // Example: route to spawn/evolve based on action
    ...
    private evolveWorld(cmd: any) {
        const delta = cmd.delta ?? 1;
        this.state.evolutionLevel += delta;

        // Simple mapping: evolutionLevel → lighting intensity
        this.state.world.lighting.intensity = Math.min(
            1,
            0.2 + this.state.evolutionLevel * 0.1
        );
    ...
    private queueActorSpawn(cmd: any) {
    ...
    private updateLighting(cmd: any) {
...
// ---------- Unreal OS Mega Bridge ----------
export function startUnrealMegaBootstrap(port: number = 7777) {
    const app = express();
    ...
    // Generic kernel command endpoint
    ...
    // Unreal-specific command endpoint
