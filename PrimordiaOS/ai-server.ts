...
interface AIRequestBody {
  prompt: string;
  task?: string;      // e.g. "caption", "script", "thumbnail", "patch"
  persona?: string;   // e.g. "creator", "analyst", "editor"
}
...
// ---- PERSONA ENGINE ----
function applyPersona(prompt: string, task?: string, persona?: string): string {
  const base = prompt;
...
function selectProvider(task?: string): ProviderName {
  // Task-based routing
  if (task === "caption") return "openai";
  if (task === "script") return "anthropic";
  if (task === "thumbnail") return "groq";
  if (task === "patch") return "anthropic";
...
async function callWithFallback(
  ...
  for (const provider of order) {
    try {
      ...
      if (provider === "lmstudio") return await callLMStudio(prompt);
